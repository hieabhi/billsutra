...
const EnhancedFolioModal = ({ bookingId, onClose, onChanged }) => {
  ...
  const handleItemSelect = (e) => {
    ...
    if (item) {
      // Calculate total tax rate from cgst + sgst (or use igst if available)
      const totalTaxRate = item.igst || (Number(item.cgst || 0) + Number(item.sgst || 0));

      setChargeForm({
        ...chargeForm,
        itemId: item._id,
        description: item.name,
        rate: Number(item.rate || 0),  // FIX: Use item.rate instead of item.price
