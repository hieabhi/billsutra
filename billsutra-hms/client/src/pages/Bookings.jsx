...
const Bookings = () => {
  ...
  const getFilteredBookings = () => {
    ...
    switch (statusFilter) {
      ...
      case 'DEPARTING':
        // Departing today - INDUSTRY STANDARD (Opera PMS, Maestro, Cloudbeds, Mews)
        // Show BOTH:
        // 1. Guests still in-house with checkout date = today (about to depart)
        // 2. Guests who already checked out today (departed today) - REGARDLESS of scheduled checkout date
        return bookings.filter(b => {
          // Still in-house, departing today (scheduled checkout)
          if (b.status === 'CheckedIn' && b.checkOutDate === today) return true;

          // Already checked out - ONLY check actualCheckOutDate (when they ACTUALLY checked out)
          // This shows early checkouts, late checkouts, and on-time checkouts ALL in "Departed Today"
          if (b.status === 'CheckedOut' && b.actualCheckOutDate) {
            const actualCheckoutDay = b.actualCheckOutDate.slice(0, 10);
            return actualCheckoutDay === today;
          }
  ...
  return (
    <div className="card">
        ...
        <div style={{ padding: 16, background: '#fed7aa', borderRadius: 8, borderLeft: '4px solid #f59e0b' }}>
          <div style={{ fontSize: 24, fontWeight: 600, color: '#b45309' }}>
            {(() => {
              const today = new Date().toISOString().slice(0, 10);
              return bookings.filter(b => {
                // Still in-house departing today
                if (b.status === 'CheckedIn' && b.checkOutDate === today) return true;
                // Already departed today
                if (b.status === 'CheckedOut' && b.actualCheckOutDate && b.actualCheckOutDate.slice(0, 10) === today) return true;
                return false;
              }).length;
            })()}
          </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, marginTop: 24, borderBottom: '2px solid #e5e7eb', flexWrap: 'nowrap', overflowX: 'auto' }}>
        <button
          type="button"
