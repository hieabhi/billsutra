...
export class Room {
  ...
  isAvailableForBooking() {
    // INDUSTRY STANDARD: Room must meet BOTH criteria to be bookable (Opera PMS, Maestro)
    // 1. Occupancy Status: Room must be AVAILABLE (no guest)
    // 2. Housekeeping Status: Room must be CLEAN (ready for guest)
    
    const bookableOccupancyStatuses = [ROOM_STATUS.AVAILABLE];
    const hasBookableOccupancy = bookableOccupancyStatuses.includes(this.status);
    
    // Room must be CLEAN for new bookings
    // DIRTY, IN_PROGRESS, MAINTENANCE rooms cannot be sold until cleaned
    const isClean = this.housekeepingStatus === HOUSEKEEPING_STATUS.CLEAN;
    
    return hasBookableOccupancy && isClean;
  }
