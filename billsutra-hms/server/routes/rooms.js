...
// INDUSTRY STANDARD: Manual sync endpoint to fix room statuses
// Follows Opera PMS pattern: rooms 2+ days away from arrival should be AVAILABLE
router.post('/sync-status', async (req, res) => {
  try {
    const { validateAndFixRoomBookingSync } = await import('../utils/roomBookingSync.js');
    const result = await validateAndFixRoomBookingSync();
    res.json({
      message: `Room status sync completed. Fixed ${result.fixed} room(s).`,
