...
export const bookingsRepo = {
  ...
  async create(data) {
    ...
    if (data.roomId) {
      ...
      // Room status check: Only block if room is in a non-bookable state
      // RESERVED is OK if no date conflict (checked above)
      // AVAILABLE, CLEAN, RESERVED are bookable
      // OCCUPIED, DIRTY, MAINTENANCE, OUT_OF_SERVICE, BLOCKED are not
      const { ROOM_STATUS } = await import('../models/Room.js');
      const bookableStatuses = [
        ROOM_STATUS.AVAILABLE,
        ROOM_STATUS.CLEAN,
        ROOM_STATUS.RESERVED // OK if no date conflict
      ];
      
      if (!bookableStatuses.includes(room.status)) {
        throw new Error(`Room ${room.number} is not available for booking (current status: ${room.status})`);
      }
    }
  ...
  async checkOut(id) {
    ...
    if (booking) {
      ...
      // INDUSTRY STANDARD: Add all folio charges to invoice (food, laundry, etc.)
      if (booking.folio?.lines && booking.folio.lines.length > 0) {
        booking.folio.lines.forEach(line => {
          // Skip room rent as it's already added above
          ...
          // Calculate tax percentages from total tax amount if available
          const totalTax = Number(line.cgst || 0) + Number(line.sgst || 0) + Number(line.igst || 0);
          const cgstPercent = line.cgstPercent || (totalTax > 0 ? (Number(line.cgst || 0) / lineAmount * 100) : cgst);
          const sgstPercent = line.sgstPercent || (totalTax > 0 ? (Number(line.sgst || 0) / lineAmount * 100) : sgst);
          const igstPercent = line.igstPercent || (totalTax > 0 ? (Number(line.igst || 0) / lineAmount * 100) : igst);
